...
int     ctl_key(int keysym, t_fractol *fractal)
{
    ...
    if (keysym == 53 || keysym == 65307)
        close_w(fractal);
    else if (keysym == 123 || keysym == 65361) //左
        fractal->shift_x -= 0.1;
    else if (keysym == 126 || keysym == 65362) //上
        fractal->shift_y += 0.1;
    else if (keysym == 124 || keysym == 65363) //右
        fractal->shift_x += 0.1;
    else if (keysym == 125 || keysym == 65364) //下
        fractal->shift_y -= 0.1;
...
int     ctl_mouse(int keysym, int x, int y, t_fractol *fractal)
{
    ...
    if (keysym == 4){ // マウスホイールを上に回したとき
        ...
        // double shift_x_original = fractal->shift_x;
        // double shift_y_original = fractal->shift_y;

        // // 2. Shift the image to the origin
        // fractal->shift_x = 0;
        // fractal->shift_y = 0;
